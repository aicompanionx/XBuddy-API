            ...
            while retries < max_retries:
                try:
                    ...
                    if retries >= max_retries:
                        logger.error(f"Function {func.__name__} execution failed, has retried {retries} times: {str(e)}")
                        break
                    
                    # Calculate delay time (exponential backoff strategy)
                    delay = min(base_delay * (2 ** (retries - 1)), max_delay)
                    logger.warning(f"Function {func.__name__} execution failed, will retry in {delay} seconds, has retried {retries} times: {str(e)}")
