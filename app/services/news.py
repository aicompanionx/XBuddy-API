...
class WebSocketConnectionManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        logger.debug(
            f"WebSocket connection established, current connection count: {len(self.active_connections)}"
        )
    ...
    def disconnect(self, websocket: WebSocket):
        if websocket in self.active_connections:
            ...
            logger.debug(
                f"WebSocket connection closed, current connection count: {len(self.active_connections)}"
            )
    ...
    async def _establish_rabbitmq_connection(self):
        ...
        try:
            connection = await aio_pika.connect_robust(
                ...
                client_properties={"connection_name": "xbuddy-websocket-client"},
            )
        ...
        while (
            ...
            try:
                ...
                (
                    ...
                    channel,
                    exchange,
                ) = await self._establish_rabbitmq_connection()
                            ...
                            async for message in queue_iter:
                                async with message.process():
                                    try:
                                        ...
                                        logger.debug(
                                            f"Received message: {body[:50]}..."
                                            if len(body) > 50
                                            else f"Received message: {body}"
                                        )
                                        ...
                                        if isinstance(data, dict):
                                            ...
                                            try:
                                                ...
                                                logger.debug(
                                                    f"Broadcaster message: {news.title[:30]}..."
                                                )
                                            except Exception as e:
                                                # If not compatible with News model, broadcast original data
                                                logger.warning(
                                                    f"Message format is not compatible with News model: {str(e)}"
                                                )
                                                await self.broadcast(data)
                                                logger.debug(
                                                    "Broadcasted custom format message"
                                                )
                                        else:
                                            logger.warning(
                                                f"Received non-dictionary message: {type(data)}"
                                            )
                                    except json.JSONDecodeError:
                                        logger.error(
                                            f"Failed to parse JSON message: {body[:50]}..."
                                        )
                                    except Exception as e:
                                        logger.error(
                                            f"Handling message failed: {str(e)}"
                                        )
                ...
                wait_time = min(
                    30, 2**self.reconnect_attempts
                )  # Exponential backoff, longest wait 30 seconds

                logger.error(f"Listening loop failed: {str(e)}")
                logger.warning(
                    f"Retry #{self.reconnect_attempts}/{self.max_reconnect_attempts} will be performed in {wait_time} seconds"
                )
                ...
                if (
                    ...
                    if self.reconnect_attempts >= self.max_reconnect_attempts:
                        logger.error(
                            f"Reached maximum retry count ({self.max_reconnect_attempts}), stopping attempts"
                        )
                    break
